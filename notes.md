...
Using the Azure Cloud Shell we can quickly create an Azure app service to host an API. I'll show you how in literally just a few steps. As you can see I'm already logged onto the Azure portal. Open the Azure Cloud Shell by clicking on its icon located on the top menu. You can maximize this window to be in full focus. To host an Azure app service we first need to create a plan. We can do this by using the AZ app service plan create command. Type az space appservice space plan space create. 
